$(function() {
    ...
    (function(){
        ...
        region.forEach(function(element){
            ...
            {
                ProvinceHtml += '<option value="' + element.provinceCode + '" selected="selected">' + element.provinceName + '</option>';

                element.mallCityList.forEach(function(child, index){

                    if(child.cityCode == $city_value)
                    {
                        CityHtml += '<option value="' + child.cityCode + '" selected="selected">' + child.cityName + '</option>';

                        child.mallAreaList.forEach(function(grandchild, i){

                            if(grandchild.areaName == $district_value)
                            {
                                DistrictHtml += '<option value="' + grandchild.areaCode + '" selected="selected">' + grandchild.areaName + '</option>';
                            }
                            else
                            {
                                DistrictHtml += "<option value='"+grandchild.areaCode+"'>"+grandchild.areaName+"</option>";
                            }
                        })
                    }
                    else
