...
use App\Repositories\DK\DKCustomerRepository;

use Response, Auth, Validator, DB, Exception;
use QrCode, Excel;

class DKCustomerController extends Controller
{
    //
    private $repo;
    public function __construct()
    {
        $this->repo = new DKCustomerRepository;
    }
    ...
    public function check_is_only_me()
    {
        ...
        {
            $me = Auth::guard('dk_customer_staff')->user();
    ...
    public function login()
    {
        ...
        {
            $view_blade = env('TEMPLATE_DK_CUSTOMER').'entrance.login';
...
//            $admin = SuperAdministrator::whereEmail($email)->first();
            ...
            $admin = DK_Customer_User::whereMobile($mobile)->first();
            ...
            if($admin)
            {
                if($admin->user_status == 1)
                {
                    ...
                    if(password_check($password,$admin->password))
                    {
                        ...
                        if($remember) Auth::guard('dk_customer_staff')->login($admin,true);
                        else Auth::guard('dk_customer_staff')->login($admin);
    ...
    {
        Auth::guard('dk_customer_staff')->user()->admin_token = '';
        Auth::guard('dk_customer_staff')->user()->save();
