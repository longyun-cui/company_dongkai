...
use App\Repositories\DK\DKCCRepository;

use Response, Auth, Validator, DB, Exception;
use QrCode, Excel;

class DKCCController extends Controller
{
    //
    private $repo;
    public function __construct()
    {
        $this->repo = new DKCCRepository;
    }
    ...
    public function check_is_only_me()
    {
        ...
        if(Auth::guard('dk_cc')->check())
        {
            ...
            if(Auth::guard('dk_cc')->user()->admin_token == $token)
            {
            ...
            if($admin)
            {
                if($admin->user_status == 1)
                {
                    ...
                    if(password_check($password,$admin->password))
                    {
                        ...
                        if($remember) Auth::guard('dk_cc')->login($admin,true);
                        else Auth::guard('dk_cc')->login($admin);
                        Auth::guard('dk_cc')->user()->login_error_num = 0;
                        Auth::guard('dk_cc')->user()->admin_token = $token;
                        Auth::guard('dk_cc')->user()->save();

                        if(Auth::guard('dk_cc')->user()->id > 10000)
                        {
...
//            }
    ...
    public function operate_company_staff_login()
    {
        ...
        if($user)
        {
            Auth::guard('dk_cc')->login($user,true);
    ...
    // 【电话数据】下载
    public function operate_pool_telephone_download_of_all()
    {
