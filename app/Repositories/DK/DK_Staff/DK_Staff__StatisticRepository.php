    ...
    public function o1__statistic__production__project($post_data)
    {
        ...
        $query_order = DK_Common__Order::withTrashed()->select('project_id','creator_team_id')
            ->addSelect(DB::raw("
                    ...
                    count(IF(inspected_result = '内部通过', TRUE, NULL)) as order_count_for_accepted_inside,
                    count(IF(inspected_result = '重复', TRUE, NULL)) as order_count_for_repeated,
                    count(IF(inspected_result = '不合格' or inspected_result2 = '不合格', TRUE, NULL)) as order_count_for_accepted_non,
                    count(IF(inspected_result = '拒绝' or inspected_result = '不合格' or inspected_result = '超区' or inspected_result = '超龄', TRUE, NULL)) as order_count_for_refused
                "))
            ->addSelect(DB::raw("
                    count(IF(is_published = 1 AND delivered_status = 1, TRUE, NULL)) as order_count_for_delivered,
...
//        dd($list->toArray());
    ...
    public function view_statistic_index($post_data)
    {
        ...
        $staff_list = DK_Common__Staff::select('id','name')->where('active',1)->where('staff_category',11)->whereIn('user_type',[11,81,82,88])->get();
        $client_list = DK_Common__Client::select('id','name')->where('active',1)->where('client_category',11)->get();
        $project_list = DK_Common__Project::select('id','name')->where('active',1)->whereIn('item_type',[1,21])->get();
        $team_list = DK_Common__Team::select('id','name')->where('active',1)->where('team_type',11)->orderby('rank','asc')->get();
