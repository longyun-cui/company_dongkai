...
class DK_Staff__DepartmentRepository {
    ...
    public function o1__department__list__datatable_query($post_data)
    {
        ...
        $query = DK_Common__Department::select(['id','item_status','name','department_category','department_type','company_id','leader_id','superior_department_id','remark','creator_id','created_at','updated_at','deleted_at'])
            ->withTrashed()
            ->with([
...
//        dd($list->toArray());
    ...
    public function o1__department__item_get($post_data)
    {
        ...
        $item = DK_Common__Department::withTrashed()
            ->with([
                'company_er'=>function($query) { $query->select(['id','name']); },
                'superior_department_er'=>function($query) { $query->select('id','name'); },
...
//            'name.unique' => '该部门号已存在！',
        ...
        $v = Validator::make($post_data, [
            ...
            'name' => 'required',
//            'name' => 'required|unique:DK_Common__Department,name',
        ], $messages);
        if ($v->fails())
        {
            $messages = $v->errors();
        ...
        {
            $is_exist = DK_Common__Department::select('id')->where('name',$post_data["name"])->count();
            if($is_exist) return response_error([],"该【部门】已存在，请勿重复添加！");

            $mine = new DK_Common__Department;
            $post_data["active"] = 1;
            $post_data["creator_id"] = $me->id;
        }
        else if($operate_type == 'edit') // 编辑
        {
            $mine = DK_Common__Department::find($operate_id);
        ...
        try
        {
            ...
            else throw new Exception("DK_Common__Department--insert--fail");
...
//            exit($e->getMessage());
    ...
    public function o1__department__item_delete($post_data)
    {
        ...
        // 判断对象是否合法
        $mine = DK_Common__Department::withTrashed()->find($item_id);
        if(!$mine) return response_error([],"该【部门】不存在，刷新页面重试！");
...
//            exit($e->getMessage());
    ...
    public function o1__department__item_enable($post_data)
    {
        ...
        // 判断对象是否合法
        $mine = DK_Common__Department::find($item_id);
        if(!$mine) return response_error([],"该【部门】不存在，刷新页面重试！");
