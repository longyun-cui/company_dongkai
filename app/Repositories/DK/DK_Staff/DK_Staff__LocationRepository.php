...
class DK_Staff__LocationRepository {
     ...
     */
    // 【地域】返回-列表-数据
    public function o1__location__list__datatable_query($post_data)
    {
        $this->get_me();
        $me = $this->me;
...
//        dd($list->toArray());
    ...
    // 【地域】获取 GET
    public function o1__location__item_get($post_data)
    {
        $messages = [
    ...
    // 【地域】保存数据
    public function o1__location__item_save($post_data)
    {
//        dd($post_data);
        $messages = [
            'operate.required' => 'operate.required.',
...
//            if(in_array($mine_data["trailer_axis_count"],["0","-1"])) unset($mine_data['trailer_axis_count']);
            ...
            else throw new Exception("DK_Common__Location--insert--fail");
...
//            exit($e->getMessage());
    ...
    // 【地域】删除
    public function o1__location__item_delete($post_data)
    {
        $messages = [
            'operate.required' => 'operate.required.',
            'item_id.required' => 'item_id.required.',
        ...
        $operate = $post_data["operate"];
        if($operate != 'location--item-delete') return response_error([],"参数【operate】有误！");
        $item_id = $post_data["item_id"];
        if(intval($item_id) !== 0 && !$item_id) return response_error([],"参数【ID】有误！");
        ...
        // 判断对象是否合法
        $mine = DK_Common__Location::withTrashed()->find($item_id);
        if(!$mine) return response_error([],"该【地域】不存在，刷新页面重试！");
...
//            exit($e->getMessage());
    ...
    // 【地域】恢复
    public function o1__location__item_restore($post_data)
    {
        $messages = [
            'operate.required' => 'operate.required.',
        ...
        $operate = $post_data["operate"];
        if($operate != 'location--item-restore') return response_error([],"参数【operate】有误！");
        $item_id = $post_data["item_id"];
        if(intval($item_id) !== 0 && !$item_id) return response_error([],"参数【ID】有误！");
...
//            exit($e->getMessage());
    ...
    // 【地域】彻底删除
    public function o1__location__item_delete_permanently($post_data)
    {
        $messages = [
            'operate.required' => 'operate.required.',
            'item_id.required' => 'item_id.required.',
        ...
        $operate = $post_data["operate"];
        if($operate != 'location--item-delete-permanently') return response_error([],"参数【operate】有误！");
        $item_id = $post_data["item_id"];
        if(intval($item_id) !== 0 && !$item_id) return response_error([],"参数【ID】有误！");
...
//            exit($e->getMessage());
    ...
    // 【地域】启用
    public function o1__location__item_enable($post_data)
    {
        $messages = [
            'operate.required' => 'operate.required.',
            'item_id.required' => 'item_id.required.',
        ...
        $operate = $post_data["operate"];
        if($operate != 'location--item-enable') return response_error([],"参数【operate】有误！");
        $item_id = $post_data["item_id"];
        if(intval($item_id) !== 0 && !$item_id) return response_error([],"参数【ID】有误！");
        ...
        // 判断对象是否合法
        $mine = DK_Common__Location::find($item_id);
        if(!$mine) return response_error([],"该【地域】不存在，刷新页面重试！");
...
//            exit($e->getMessage());
    ...
    // 【地域】禁用
    public function o1__location__item_disable($post_data)
    {
        $messages = [
            'operate.required' => 'operate.required.',
            'item_id.required' => 'item_id.required.',
        ...
        $operate = $post_data["operate"];
        if($operate != 'location--item-disable') return response_error([],"参数【operate】有误！");
        $item_id = $post_data["item_id"];
        if(intval($item_id) !== 0 && !$item_id) return response_error([],"参数【ID】有误！");
...
//            exit($e->getMessage());
    ...
    // 【地域】【操作记录】返回-列表-数据
    public function o1__location__item_operation_record_list__datatable_query($post_data)
    {
        $this->get_me();
        $me = $this->me;
