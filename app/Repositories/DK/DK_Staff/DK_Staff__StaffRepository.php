...
class DK_Staff__StaffRepository {
...
//        dd($list->toArray());
    ...
    public function o1__staff__item_get($post_data)
    {
        ...
        $item = DK_Common__Staff::withTrashed()
            ->with([
                'company_er'=>function($query) { $query->select('id','name'); },
                'department_er'=>function($query) { $query->select('id','name'); },
                'team_er'=>function($query) { $query->select('id','name'); },
...
//            $post_data['name'] = $post_data['name'];
        ...
        try
        {
            ...
            if($bool)
            {
                ...
                {
//                    $user_ext = new DK_Common__Staff_Ext;
...
//                    $result = upload_storage($post_data["portrait"], null, null, 'assign');
            ...
            else throw new Exception("DK_Common__Staff--insert--fail");
...
//            exit($e->getMessage());
    ...
    public function o1__staff__item_delete($post_data)
    {
        ...
        // 判断对象是否合法
        $mine = DK_Common__Staff::withTrashed()->find($item_id);
        if(!$mine) return response_error([],"该【员工】不存在，刷新页面重试！");
        ...
        try
        {
            ...
            if(!$bool) throw new Exception("DK_Common__Staff--delete--fail");
            else
            {
...
//            exit($e->getMessage());
    ...
    public function o1__staff__item_restore($post_data)
    {
        ...
        try
        {
            $mine->timestamps = false;
            $bool = $mine->restore();
            if(!$bool) throw new Exception("DK_Common__Staff--restore--fail");
            else
            {
                $staff_operation_record = new DK_Common__Record__by_Operation;
                $bool_sop = $staff_operation_record->fill($record_data)->save();
...
//            exit($e->getMessage());
    ...
    public function o1__staff__item_password_reset($post_data)
    {
        ...
        // 判断对象是否合法
        $mine = DK_Common__Staff::withTrashed()->find($id);
        if(!$mine) return response_error([],"该【员工】不存在，刷新页面重试！");
