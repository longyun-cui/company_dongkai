    ...
    public function o1__api__ai_inspecting__from__ali($post_data)
    {
        ...
        $data = [
            ...
            "messages" => [
                [
                    "role" => "system",
                    "content" => "你是一个严格的质检员，请分析录音内容，请严格遵循JSON Schema输出。禁止使用Markdown格式，禁止包含json代码块标记，禁止换行，输出内容必须是单行的紧凑JSON字符串，每一个返回字段均为关联数组类型， :之后不要再有{，输出内容也不要包含英文字符的单引号与双引号，如需引用标注，请使用中文的引号，避免数据格式混乱。不要包含任何其他解释或文字。如果信息在录音中不存在，请对应字段填null！"
                ],
                [
                    "role" => "user",
