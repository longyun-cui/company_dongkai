...
class DK_Staff__CompanyRepository {
     ...
     */
    // 【公司】返回-列表-数据
    public function o1__company__list__datatable_query($post_data)
    {
        $this->get_me();
        $me = $this->me;
        ...
        {
            $query->where('superior_company_id',$me->company_district_id);
            $query->whereNotIn('superior_company_id', [-1,0]);
        }
...
//        dd($list->toArray());
        ...
        foreach($list as $k => $v)
        {
            if($v->company_type == 11)
            {
                $v->district_id = $v->id;
            }
            else if($v->company_type == 21)
            {
...
//        dd($list->toArray());
    ...
    // 【公司】获取 GET
    public function o1__company__item_get($post_data)
    {
        $messages = [
        ...
        $item = DK_Common__Company::withTrashed()
            ->with([
    ...
    // 【公司】保存 SAVE
    public function o1__company__item_save($post_data)
    {
        $messages = [
            'operate.required' => 'operate.required.',
            'company_category.required' => '请选择公司类型！',
...
//            'name.unique' => '该公司号已存在！',
        ...
        $v = Validator::make($post_data, [
            ...
            'name' => 'required',
//            'name' => 'required|unique:DK_Common__Company,name',
        ], $messages);
        if ($v->fails())
        {
            $messages = $v->errors();
        ...
        {
            $is_exist = DK_Common__Company::select('id')->where('name',$post_data["name"])->count();
            if($is_exist) return response_error([],"该【公司】已存在，请勿重复添加！");

            $mine = new DK_Common__Company;
            $post_data["active"] = 1;
            $post_data["creator_id"] = $me->id;
        }
        else if($operate_type == 'edit') // 编辑
        {
            $mine = DK_Common__Company::find($operate_id);
            if(!$mine) return response_error([],"该【公司】不存在，刷新页面重试！");
        }
...
//            {
//                $mine_data['superior_company_id'] = $me->company_id;
//            }
            ...
            else throw new Exception("DK_Common__Company--insert--fail");
...
//            exit($e->getMessage());
    ...
    // 【公司】删除
    public function o1__company__item_delete($post_data)
    {
        $messages = [
            'operate.required' => 'operate.required.',
            'item_id.required' => 'item_id.required.',
        ...
        $operate = $post_data["operate"];
        if($operate != 'company--item-delete') return response_error([],"参数【operate】有误！");
        ...
        // 判断对象是否合法
        $mine = DK_Common__Company::withTrashed()->find($item_id);
        if(!$mine) return response_error([],"该【公司】不存在，刷新页面重试！");
        ...
        try
        {
            ...
            if(!$bool) throw new Exception("DK_Common__Company--delete--fail");
            else
            {
...
//            exit($e->getMessage());
    ...
    // 【公司】恢复
    public function o1__company__item_restore($post_data)
    {
        $messages = [
            'operate.required' => 'operate.required.',
        ...
        $operate = $post_data["operate"];
        if($operate != 'company--item-restore') return response_error([],"参数【operate】有误！");
        $item_id = $post_data["item_id"];
        if(intval($item_id) !== 0 && !$item_id) return response_error([],"参数【ID】有误！");
        ...
        try
        {
            $mine->timestamps = false;
            $bool = $mine->restore();
            if(!$bool) throw new Exception("DK_Common__Company--restore--fail");
            else
            {
                $staff_operation_record = new DK_Common__Record_by_Operation;
                $bool_sop = $staff_operation_record->fill($record_data)->save();
...
//            exit($e->getMessage());
    ...
    // 【公司】彻底删除
    public function o1__company__item_delete_permanently($post_data)
    {
        $messages = [
            'operate.required' => 'operate.required.',
            'item_id.required' => 'item_id.required.',
        ...
        $operate = $post_data["operate"];
        if($operate != 'company--item-delete-permanently') return response_error([],"参数【operate】有误！");
        $item_id = $post_data["item_id"];
        if(intval($item_id) !== 0 && !$item_id) return response_error([],"参数【ID】有误！");
...
//            exit($e->getMessage());
    ...
    // 【公司】启用
    public function o1__company__item_enable($post_data)
    {
        $messages = [
            'operate.required' => 'operate.required.',
            'item_id.required' => 'item_id.required.',
        ...
        $operate = $post_data["operate"];
        if($operate != 'company--item-enable') return response_error([],"参数【operate】有误！");
        $item_id = $post_data["item_id"];
        if(intval($item_id) !== 0 && !$item_id) return response_error([],"参数【ID】有误！");
        ...
        // 判断对象是否合法
        $mine = DK_Common__Company::find($item_id);
        if(!$mine) return response_error([],"该【公司】不存在，刷新页面重试！");
        ...
        try
        {
            ...
            $mine->timestamps = false;
            $bool = $mine->save();
            if(!$bool) throw new Exception("DK_Common__Company--update--fail");
            else
            {
                $staff_operation_record = new DK_Common__Record_by_Operation;
                $bool_sop = $staff_operation_record->fill($record_data)->save();
...
//            exit($e->getMessage());
    ...
    // 【公司】禁用
    public function o1__company__item_disable($post_data)
    {
        $messages = [
            'operate.required' => 'operate.required.',
            'item_id.required' => 'item_id.required.',
        ...
        $operate = $post_data["operate"];
        if($operate != 'company--item-disable') return response_error([],"参数【operate】有误！");
        $item_id = $post_data["item_id"];
        if(intval($item_id) !== 0 && !$item_id) return response_error([],"参数【ID】有误！");
...
//            exit($e->getMessage());
    ...
    // 【公司】【操作记录】返回-列表-数据
    public function o1__company__item_operation_record_list__datatable_query($post_data)
    {
        $this->get_me();
        $me = $this->me;
