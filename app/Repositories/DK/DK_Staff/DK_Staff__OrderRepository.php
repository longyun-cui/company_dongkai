...
//        if(in_array($me->staff_position,[99]) && $item->creator_id != $me->id) return response_error([],"该内容不是你的，你不能操作！");
        ...
        // [判断]交付结果
        if(in_array($delivered_result,['交付','折扣交付','郊区交付','内部交付']))
        {
            // 立即交付
            $is_next = 1;
            $is_delivery = 1;
...
//                return response_error([],"交付【客户】不存在！");
        ...
        $record_data["order_id"] = $id;
        $record_data["operate_object"] = 1;
        $record_data["operate_category"] = 61;
        $record_data["operate_type"] = 1;
...
//            if($project->client_id != 0) $client_id = $project->client_id;
        ...
        // 是否已经交付
        $is_order_list = DK_Common__Order::with('project_er')->where(['client_phone'=>$client_phone,'delivered_result'=>'已交付'])->get();
        if(count($is_order_list) > 0)
        {
            foreach($is_order_list as $o)
            {
...
//                  $data = $reader->toArray();
                ...
                try
                {
                    ...
                    foreach($order_data as $key => $value)
                    {
                        ...
                        $is_repeat = DK_Common__Order::where(['project_id'=>$project_id,'client_phone'=>$value['client_phone']])
