...
//            exit($e->getMessage());
    ...
    // 【拨号】返回-列表-视图
    public function view_call_call_list($post_data)
    {
        $this->get_me();
        $me = $this->me;
...
//        dd($view_data);
        ...
        $view_data['menu_active_of_call_list'] = 'active menu-open';

        $view_blade = env('TEMPLATE_DK_ADMIN_2').'entrance.call.call-list';
        return view($view_blade)->with($view_data);
    }
    // 【拨号】返回-列表-数据
    public function get_call_call_list_datatable($post_data)
    {
        $this->get_me();
        $me = $this->me;
...
//        }
        ...
        // 拨号结果
        if(isset($post_data['call_result']))
        {
            if(in_array($post_data['call_result'],["全部","成功","失败"]))
            {
                if($post_data['call_result'] == "成功")
                {
                    $query->where('timeLength', '>', 0);
                }
                else if($post_data['call_result'] == "失败")
                {
                    $query->where('timeLength', 0);
                }
            }
