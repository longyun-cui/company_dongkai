...
//            ->first();
        ...
        $funds_available = floatval($funds_recharge_total - $funds_consumption_total - $funds_obligation_total);
        ...
        $return['funds_available'] = format_number($funds_available);
...
//            ->selectAdd(DB::Raw("FROM_UNIXTIME(assign_time, '%Y-%m-%d') as assign_date"))
        ...
        {
            $query->where('customer_staff_id',$me->id);
        }
...
//            ->selectAdd(DB::Raw("FROM_UNIXTIME(assign_time, '%Y-%m-%d') as assign_date"))
        ...
        if(!empty($post_data['assign'])) $query->whereDate(DB::Raw("from_unixtime(purchased_at)"), $post_data['assign']);
        if(!empty($post_data['assign_start'])) $query->whereDate(DB::Raw("from_unixtime(assign_time)"), '>=', $post_data['assign_start']);
        if(!empty($post_data['assign_ended'])) $query->whereDate(DB::Raw("from_unixtime(assign_time)"), '<=', $post_data['assign_ended']);
        ...
        else $query->orderBy("purchased_at", "desc");
