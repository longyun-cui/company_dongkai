...
//        dd($list->toArray());
    ...
    // 【用户】【客户】返回-列表-视图
    public function view_user_customer_list_for_all($post_data)
    {
        $this->get_me();

        $return['menu_active_of_customer_list_for_all'] = 'active menu-open';
        $view_blade = env('TEMPLATE_DK_SUPER').'entrance.user.customer-list-for-all';
        return view($view_blade)->with($return);
    }
    // 【用户】【客户】返回-列表-数据
    public function get_user_customer_list_for_all_datatable($post_data)
    {
        $this->get_me();
        $me = $this->me;
...
//        dd($list->toArray());
    ...
    // 【用户】【客户】返回-列表-视图
    public function view_user_customer_staff_list($post_data)
    {
        $this->get_me();

        $return['menu_active_of_customer_staff_list'] = 'active menu-open';
        $view_blade = env('TEMPLATE_DK_SUPER').'entrance.user.customer-staff-list';
        return view($view_blade)->with($return);
    }
    // 【用户】【客户】返回-列表-数据
    public function get_user_customer_staff_list_datatable($post_data)
    {
        $this->get_me();
        $me = $this->me;

        $query = DK_Customer_User::select('*')
//            ->with(['district'])
            ->whereIn('user_category',[11])
            ->whereIn('user_type',[0,1,9,11,19,21,22,41,61,71,77,81,84,88]);
//            ->whereHas('fund', function ($query1) { $query1->where('totalfunds', '>=', 1000); } )
