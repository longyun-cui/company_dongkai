...
//            ->withTrashed()
        ...
        else $query->orderBy("rank", "asc")->orderBy("id", "asc");
...
//            exit($e->getMessage());
    ...
    public function operate_pool_telephone_download_of_second($post_data)
    {
        $messages = [
            'operate.required' => 'operate.required.',
            'pool_id.required' => '请选择城市！',
            'second.required' => '请选择指定秒数！',
        ];
        $v = Validator::make($post_data, [
            'operate' => 'required',
            'pool_id' => 'required',
        ...
        $operate = $post_data["operate"];
        if($operate != 'telephone-download-of-second') return response_error([],"参数[operate]有误！");
        ...
        $name = $pool_name.'-'.$date_Ymd.'-'.$second.'秒';
...
//            $task_insert['tag'] = $tag;
            ...
            $telephone_list = ($modal ?? false)::select('p.phone')
                ->from("{$table} as p")
                ->join('a_cdr as cdr', 'p.phone', '=', 'cdr.phone')
                ->where('cdr.holdtime', '>=', $second)
                ->distinct()
                ->get() ?: collect();
...
//        if(!in_array($me->user_type,[0,1,9])) return view(env('TEMPLATE_ROOT_FRONT').'errors.404');
    ...
    // 【电话池】返回-编辑-视图
    public function view_service_telephone_edit($post_data)
    {
        $this->get_me();
        $me = $this->me;
        if(!in_array($me->user_type,[0,1,11,21,22])) return view(env('TEMPLATE_DK_CC').'errors.404');
    ...
    // 【电话池】保存-数据
    public function operate_service_telephone_save($post_data)
    {
        $messages = [
            'operate.required' => 'operate.required',
            'title.required' => '请输入标题！',
...
//            ->withTrashed()
        ...
        {
            if(!in_array($post_data['call_record_service'],[-1,0,-9,'-1','0','-9']))
            {
