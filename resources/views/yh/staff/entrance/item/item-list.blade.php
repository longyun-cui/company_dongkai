...
@section('head_title')
    @if(in_array(env('APP_ENV'),['local'])){{ $local or '【l】' }}@endif{{ $head_title or '内容' }} - 员工系统 - {{ config('info.info.short_name') }}
@endsection
                ...
                <div class="tab-content" style="width:100%; padding:10px 0;float:left;">
                    <div class="active tab-pane" id="all">
                        @if(!empty($item_list) && count($item_list))
                            @include(env('TEMPLATE_YH_STAFF').'component.item-list',['item_list'=>$item_list])
                        @endif
                    </div>
