...
@section('header','')
@section('description'){{ $title_text or '数据导出' }} - 管理员系统 - {{ config('info.info.short_name') }}@endsection
@section('breadcrumb')
...
{{--                        </select>--}}
                        ...
                        <select class="form-control form-filter" name="order-quality" style="width:100px;">
                            <option value ="-1">工单质量</option>
                            @foreach(config('info.order_quality') as $v)
...
@include(env('TEMPLATE_DK_CUSTOMER').'entrance.statistic.statistic-export-script')
<script>
    $(function(){
...
//                            return data;
                    ...
                    },
                    {
                        ...
                        "orderable": false,
                        render: function(data, type, row, meta) {
                            return row.client_creator == null ? '未知' : '<a href="javascript:void(0);">'+row.client_creator.username+'</a>';
                        }
                    },
                    {
                        "className": "",
