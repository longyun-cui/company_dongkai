...
{{--                        <input type="text" class="form-control form-filter filter-keyup date_picker" name="order-delivered_date" placeholder="交付日期" value="" readonly="readonly" style="width:100px;text-align:center;" />--}}
                        ...
                        @if(in_array($me->user_type,[0,1,9,11,61,66]))
                            <select class="form-control form-filter select2-box order-select2-customer" name="order-customer" style="width:160px;">
...
{{--                            <option value="已交付" @if("已交付" == $delivered_status) selected="selected" @endif>已交付</option>--}}
                        ...
                        <input type="text" class="form-control form-filter filter-keyup" name="order-customer-name" placeholder="客户姓名" value="{{ $customer_name or '' }}" style="width:88px;" />
                        <input type="text" class="form-control form-filter filter-keyup" name="order-customer-phone" placeholder="客户电话" value="{{ $customer_phone or '' }}" style="width:88px;" />
            ...
            <form action="" method="post" class="form-horizontal form-bordered " id="modal-deliver-select-set-form">
                <div class="box-body">
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-md-8 " id="deliver-set-distributed-customer-list">
                        </div>
                    </div>
                    <div class="form-group">
                        <label class="control-label col-md-2">选择客户</label>
                        <div class="col-md-8 ">
                            <select class="form-control select2-box" name="deliver-set-customer-id" style="width:48%;" id="">
                                <option value="-1">选择客户</option>
                                @foreach($customer_list as $v)
                                    <option value="{{ $v->id }}">{{ $v->username }}</option>
                                @endforeach
...
<div class="modal fade modal-main-body" id="modal-body-for-order-create">
    <div class="col-md-9 col-md-offset-2 margin-top-64px margin-bottom-64px bg-white">
        <div class="box- box-info- form-container">
            ...
            @include(env('TEMPLATE_DK_ADMIN_2').'component.order-create')
...
//                "aLengthMenu": [[20, 50, 200, 500, -1], ["20", "50", "200", "500", "全部"]],
                ...
                "ajax": {
                    ...
                    "data": function (d) {
                        ...
                        d.customer = $('select[name="order-customer"]').val();
                        d.status = $('select[name="order-status"]').val();
                        d.order_type = $('select[name="order-type"]').val();
                        d.customer_name = $('input[name="order-customer-name"]').val();
                        d.customer_phone = $('input[name="order-customer-phone"]').val();
                            ...
                            else
                            {
                                ...
                                if("{{ in_array($me->user_type,[0,1,11,61,66,71,77]) }}")
                                {
                                    ...
                                    if(row.sale_status == 1)
                                    {
                                        ...
                                        $html_deliver = '<a class="btn btn-xs bg-yellow item-put-on-show" data-id="'+data+'">上架</a>';
                                    }
                                    else
                                    {
                                        // $html_deliver = '<a class="btn btn-xs bg-green disabled- item-deliver-submit" data-id="'+data+'">再交4</a>';
                                        $html_deliver = '<a class="btn btn-xs bg-yellow item-put-on-show" data-id="'+data+'">重交</a>';
                                    }
...
//                    });
                    ...
                    if($('select[name="order-customer"]').val() > 0)  $obj.customer_id = $('select[name="order-customer"]').val();
                    if($('select[name="order-project"]').val() > 0)  $obj.project_id = $('select[name="order-project"]').val();
                    if($('input[name="order-customer-name"]').val())  $obj.customer_name = $('input[name="order-customer-name"]').val();
                    if($('input[name="order-customer-phone"]').val())  $obj.customer_phone = $('input[name="order-customer-phone"]').val();
...
@include(env('TEMPLATE_DK_ADMIN_2').'component.order-create-script')
@endsection
