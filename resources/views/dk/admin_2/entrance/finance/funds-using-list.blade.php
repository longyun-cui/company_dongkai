...
{{--                        <input type="text" class="form-control form-filter item-search-keyup" name="title" placeholder="名目" style="width:88px;" />--}}
                        ...
                        @if(in_array($me->user_type,[0,1,9,11,61,66]))
                            <select class="form-control form-filter select2-box" name="finance-customer" style="width:160px;">
...
//                "aLengthMenu": [[20, 50, 200, 500, -1], ["20", "50", "200", "500", "全部"]],
                ...
                "ajax": {
                    ...
                    "data": function (d) {
                        ...
                        d.customer = $('select[name="finance-customer"]').val();
...
//                            return $year+'-'+$month+'-'+$day+'&nbsp;&nbsp;'+$hour+':'+$minute+':'+$second;
                    ...
                    },
                    {
                        ...
                        render: function(data, type, row, meta) {
                            if(data == 71)
                            {
                                if(row.choice_er == null) return '线索';
                                else
                                {
                                    var $result_html = '';
                                    if(row.choice_er.sale_type == 1) $result_html = '<small class="btn-xs bg-blue">普通·线索</small>';
                                    else if(row.choice_er.sale_type == 11) $result_html = '<small class="btn-xs bg-green">优选·线索</small>';
                                    else if(row.choice_er.sale_type == 66) $result_html = '<small class="btn-xs bg-yellow">独享·线索</small>';
                    ...
                    },
                    {
                        ...
                        "orderable": false,
                        render: function(data, type, row, meta) {
                            if(data == 71) return '<a href="javascript:void(0);">'+row.choice_id+'</a>';
...
@include(env('TEMPLATE_DK_ADMIN_2').'entrance.finance.funds-using-list-script')
@endsection
