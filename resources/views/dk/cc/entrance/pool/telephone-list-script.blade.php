...
//            $('select.form-filter').selectpicker('refresh');
        ...
        $(".main-content").on('click', ".item-down-all-submit", function() {
            var $that = $(this);
            ...
            $.post(
                "{{ url('/pool/telephone-download-of-all') }}",
                {
                    _token: $('meta[name="_token"]').attr('content'),
                    operate: "telephone-download-of-all",
                    pool_id: $pool_id
                },
                'json'
            )
        ...
        $(".main-content").on('click', ".item-detail-modal-show", function() {
            ...
            $modal.find('.item-detail-0 .detail-text').html(formatNumberWithCommas($info.data('score-0')));
