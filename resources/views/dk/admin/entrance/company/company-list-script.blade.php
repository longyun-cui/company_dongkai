<script>
    $(function() {
        ...
        $(".main-content").on('click', ".filter-cancel", function() {
            $("#datatable-for-company-list").find('textarea.form-filter, input.form-filter, select.form-filter').each(function () {
                $(this).val("");
            });

//            $('select.form-filter').selectpicker('refresh');
            $("#datatable-for-company-list").find('select.form-filter option').attr("selected",false);
            $("#datatable-for-company-list").find('select.form-filter').find('option:eq(0)').attr('selected', true);
        ...
        $(".main-content").on('click', ".filter-empty", function(e) {
            ...
            $("#datatable-search-for-company-list").find('textarea.form-filter, input.form-filter, select.form-filter').each(function () {
                $(this).val("");
            });
...
//            $('select.form-filter').selectpicker('refresh');
            $("#datatable-search-for-company-list").find('select.form-filter option').attr("selected",false);
            $("#datatable-search-for-company-list").find('select.form-filter').find('option:eq(0)').attr('selected', true);
        });
        // 【查询】回车
        $(".main-content").on('keyup', ".filter-keyup", function(event) {
            if(event.keyCode ==13)
            {
                $("#datatable-search-for-company-list").find(".filter-submit").click();
            }
        });
        ...
        $(".main-content").on('click', ".item-edit-link", function() {
            var $that = $(this);
            window.location.href = "/company/company-edit?id="+$that.attr('data-id');
        });
        ...
        $(".main-content").on('click', ".item-admin-delete-submit", function() {
            ...
            layer.msg('确定"删除"么？', {
                ...
                ,yes: function(index){
                    $.post(
                        "{{ url('/company/company-admin-delete') }}",
                        {
                            _token: $('meta[name="_token"]').attr('content'),
        ...
        $(".main-content").on('click', ".item-admin-restore-submit", function() {
            ...
            layer.msg('确定"恢复"么？', {
                ...
                ,yes: function(index){
                    $.post(
                        "{{ url('/company/company-admin-restore') }}",
                        {
                            _token: $('meta[name="_token"]').attr('content'),
        ...
        $(".main-content").on('click', ".item-admin-delete-permanently-submit", function() {
            ...
            layer.msg('确定"永久删除"么？', {
                ...
                ,yes: function(index){
                    $.post(
                        "{{ url('/company/company-admin-delete-permanently') }}",
                        {
                            _token: $('meta[name="_token"]').attr('content'),
        ...
        $(".main-content").on('click', ".item-admin-enable-submit", function() {
            ...
            layer.msg('确定"启用"？', {
                ...
                ,yes: function(index){
                    $.post(
                        "{{ url('/company/company-admin-enable') }}",
                        {
                            _token: $('meta[name="_token"]').attr('content'),
        ...
        $(".main-content").on('click', ".item-admin-disable-submit", function() {
            ...
            layer.msg('确定"禁用"？', {
                ...
                ,yes: function(index){
                    $.post(
                        "{{ url('/company/company-admin-disable') }}",
                        {
                            _token: $('meta[name="_token"]').attr('content'),
        ...
        $(".main-content").on('click', "#item-submit-for-info-text-set", function() {
            ...
            // });
                    ...
                    $.post(
                        "{{ url('/company/company-info-text-set') }}",
                        {
                            _token: $('meta[name="_token"]').attr('content'),
...
//                                $('#datatable_ajax_inner').DataTable().ajax.reload(null, false);
        ...
        $(".main-content").on('click', "#item-submit-for-info-time-set", function() {
            ...
            // });
                    ...
                    $.post(
                        "{{ url('/company/company-info-text-set') }}",
                        {{--"{{ url('/company/company-info-time-set') }}",--}}
                        {
                            _token: $('meta[name="_token"]').attr('content'),
...
//            $('select[name=info-select-set-column-value]').find("option").eq(0).attr("selected","selected");
        ...
        $(".main-content").on('click', "#item-submit-for-info-select-set", function() {
            ...
            // });
                    ...
                    $.post(
                        "{{ url('/company/company-info-select-set') }}",
                        {
                            _token: $('meta[name="_token"]').attr('content'),
...
//                                $('#datatable_ajax_inner').DataTable().ajax.reload(null, false);
        ...
        $(".main-content").on('dblclick', ".modal-show-for-attachment", function() {
            ...
            $.ajax({
                ...
                dataType:'json',
                async:false,
                url: "{{ url('/company/company-get-attachment-html') }}",
                data: {
                    _token: $('meta[name="_token"]').attr('content'),
                    operate:"item-get",
                    item_id: $that.attr('data-id')
        ...
        $(".main-content").on('click', "#item-submit-for-attachment-set", function() {
            ...
            var index1 = layer.load(1, {
                ...
                success: function (layer) {
                    ...
                    var options = {
                        url: "{{ url('/company/company-info-attachment-set') }}",
                        type: "post",
                        dataType: "json",
                        // target: "#div2",
                        success: function (data) {
...
//                                $('#datatable_ajax_inner').DataTable().ajax.reload(null, false);
        ...
        $(".main-content").on('click', ".item-attachment-delete-this", function() {
            ...
            layer.msg('确定"删除"么？', {
                ...
                ,yes: function(index){
                    $.post(
                        "{{ url('/company/company-info-attachment-delete') }}",
                        {
                            _token: $('meta[name="_token"]').attr('content'),
...
//                                $('#datatable_ajax').DataTable().ajax.reload(null,false);
        ...
        $(".main-content").on('click', ".item-modal-show-for-recharge", function() {
            ...
            var $name = $row.find('.company-name').html();


            $('input[name="finance-create-company-id"]').val($id);
            $('.finance-create-company-name').html($name);
                ...
                ,yes: function(index){
                    ...
                    $.post(
                        "{{ url('/company/company-finance-recharge-create') }}",
                        {
                            ...
                            company_id: $('input[name="finance-create-company-id"]').val(),
