<script>
    $(function() {
        ...
        // 【搜索】
        $("#datatable-for-district-list").on('click', ".filter-submit", function() {
            $('#datatable_ajax').DataTable().ajax.reload();
        });
        // 【重置】
        $("#datatable-for-district-list").on('click', ".filter-cancel", function() {
            $("#datatable-for-district-list").find('textarea.form-filter, input.form-filter, select.form-filter').each(function () {
                $(this).val("");
            });

//            $('select.form-filter').selectpicker('refresh');
            $("#datatable-for-district-list").find('select.form-filter option').attr("selected",false);
            $("#datatable-for-district-list").find('select.form-filter').find('option:eq(0)').attr('selected', true);

            $('#datatable_ajax').DataTable().ajax.reload();
        });
        // 【查询】回车
        $("#datatable-for-district-list").on('keyup', ".item-search-keyup", function(event) {
            if(event.keyCode ==13)
            {
                $("#datatable-for-district-list").find(".filter-submit").click();
            }
        });
        ...
        $(".main-content").on('click', ".item-edit-link", function() {
            var $that = $(this);
            window.location.href = "/district/district-edit?id="+$that.attr('data-id');
        });
        ...
        $(".main-content").on('click', ".item-admin-delete-submit", function() {
            ...
            layer.msg('确定"删除"么？', {
                ...
                ,yes: function(index){
                    $.post(
                        "{{ url('/district/district-admin-delete') }}",
                        {
                            _token: $('meta[name="_token"]').attr('content'),
                            operate: "district-admin-delete",
        ...
        $(".main-content").on('click', ".item-admin-restore-submit", function() {
            ...
            layer.msg('确定"恢复"么？', {
                ...
                ,yes: function(index){
                    $.post(
                        "{{ url('/district/district-admin-restore') }}",
                        {
                            _token: $('meta[name="_token"]').attr('content'),
                            operate: "district-admin-restore",
                            item_id: $that.attr('data-id')
                        },
                        function(data){
                            layer.close(index);
        ...
        $(".main-content").on('click', ".item-admin-delete-permanently-submit", function() {
            ...
            layer.msg('确定"永久删除"么？', {
                ...
                ,yes: function(index){
                    $.post(
                        "{{ url('/district/district-admin-delete-permanently') }}",
                        {
                            _token: $('meta[name="_token"]').attr('content'),
                            operate: "district-admin-delete-permanently",
                            item_id: $that.attr('data-id')
                        },
                        function(data){
                            layer.close(index);
        ...
        $(".main-content").on('click', ".item-admin-enable-submit", function() {
            ...
            layer.msg('确定"启用"？', {
                ...
                ,yes: function(index){
                    ...
                    $.post(
                        "{{ url('/district/district-admin-enable') }}",
                        {
                            _token: $('meta[name="_token"]').attr('content'),
                            operate: "district-admin-enable",
                            item_id: $that.attr('data-id')
                        },
                        function(data){
                            layer.close(index);
        ...
        $(".main-content").on('click', ".item-admin-disable-submit", function() {
            ...
            layer.msg('确定"禁用"？', {
                ...
                ,yes: function(index){
                    ...
                    $.post(
                        "{{ url('/district/district-admin-disable') }}",
                        {
                            _token: $('meta[name="_token"]').attr('content'),
                            operate: "district-admin-disable",
                            item_id: $that.attr('data-id')
                        },
                        function(data){
                            layer.close(index);
        ...
        $(".main-content").on('click', "#item-submit-for-info-text-set", function() {
            ...
            // });
                    ...
                    $.post(
                        "{{ url('/district/district-info-text-set') }}",
                        {
                            _token: $('meta[name="_token"]').attr('content'),
...
//                                $('#datatable_ajax_inner').DataTable().ajax.reload(null, false);
        ...
        $(".main-content").on('click', "#item-submit-for-info-time-set", function() {
            ...
            // });
                    ...
                    $.post(
                        "{{ url('/district/district-info-text-set') }}",
                        {{--"{{ url('/district/district-info-time-set') }}",--}}
                        {
                            _token: $('meta[name="_token"]').attr('content'),
...
//            $('select[name=info-select-set-column-value]').find('option').eq(0).attr('data-id',$that.attr("data-value"));
            ...
            $('select[name=info-select-set-column-value]').removeClass('select2-district').removeClass('select2-client');
...
//            $('select[name=info-select-set-column-value]').find("option").eq(0).attr("selected","selected");
        ...
        $(".main-content").on('click', "#item-submit-for-info-select-set", function() {
            ...
            // });
                    ...
                    $.post(
                        "{{ url('/district/district-info-select-set') }}",
                        {
                            _token: $('meta[name="_token"]').attr('content'),
...
//                                $('#datatable_ajax_inner').DataTable().ajax.reload(null, false);
        ...
        $(".main-content").on('dblclick', ".modal-show-for-attachment", function() {
            ...
            $('input[name=attachment-set-district-id]').val($that.attr("data-id"));
            ...
            $.ajax({
                ...
                dataType:'json',
                async:false,
                url: "{{ url('/district/district-get-attachment-html') }}",
                data: {
                    _token: $('meta[name="_token"]').attr('content'),
                    operate:"item-get",
                    item_id: $that.attr('data-id')
        ...
        $(".main-content").on('click', "#item-submit-for-attachment-set", function() {
            ...
            var index1 = layer.load(1, {
                ...
                success: function (layer) {
                    ...
                    var options = {
                        url: "{{ url('/district/district-info-attachment-set') }}",
                        type: "post",
                        dataType: "json",
                        // target: "#div2",
                        success: function (data) {
...
//                                $('#datatable_ajax_inner').DataTable().ajax.reload(null, false);
        ...
        $(".main-content").on('click', ".item-attachment-delete-this", function() {
            ...
            layer.msg('确定"删除"么？', {
                ...
                ,yes: function(index){
                    $.post(
                        "{{ url('/district/district-info-attachment-delete') }}",
                        {
                            _token: $('meta[name="_token"]').attr('content'),
                            operate: "district-attachment-delete",
                            item_id: $that.attr('data-id')
                        },
                        function(data){
                            layer.close(index);
