<script>
    $(function() {
        ...
        $(".main-content").on('click', ".filter-submit", function() {
            ...
            if($.fn.DataTable.isDataTable('#datatable-for-statistic-company-daily'))
            {
                console.log('DataTable 已初始化');
                $('#datatable-for-statistic-company-daily').DataTable().ajax.reload();
            }
            else
            {
        ...
        $('.main-content').on('click', ".filter-cancel", function() {
            ...
            $('#datatable-search-for-statistic-company-daily').find('textarea.form-filter, input.form-filter, select.form-filter').each(function () {
                $(this).val("");
            });

//            $('select.form-filter').selectpicker('refresh');
            $('#datatable-search-for-statistic-company-daily').find('select.form-filter option').attr("selected",false);
            $('#datatable-search-for-statistic-company-daily').find('select.form-filter').find('option:eq(0)').attr('selected', true);

            $('#datatable-search-for-statistic-company-daily').find('input[name=statistic-company-daily-time-type]').val('');

            var $month_dom = $('input[name="statistic-company-daily-month"]');
            var $month_default = $month_dom.attr('data-default')
            $month_dom.val($month_default);

            var $date_dom = $('input[name="statistic-company-daily-date"]');
