<script>
    function Datatable__for__Location_List($tableId)
    {
        let $that = $($tableId);
...
//                "scrollY": true,
            "scrollCollapse": true,
            "ajax": {
                'url': "{{ url('/o1/location/location-list/datatable-query') }}",
                "type": 'POST',
                "dataType" : 'json',
                "data": function (d) {
...
//                    },
                ...
                //             else $(nTd).attr('data-operate-type','add');
                //
                //             if(row.location_type == 11)
                //             {
                //                 $(nTd).attr('data-location-type','manager');
                //             }
                //             else if(row.location_type == 21)
                //             {
                //                 $(nTd).attr('data-location-type','supervisor');
                //             }
                //
                //         }
                //     },
...
//                            return $year+'-'+$month+'-'+$day+'&nbsp;&nbsp;'+$hour+':'+$minute+':'+$second;
                ...
                },
                {
                    ...
                    render: function(data, type, row, meta) {
                        ...
                        var $html_able = '';
                        var $html_delete = '';
                        var $html_operation_record = '<a class="btn btn-xs modal-show--for--location--item-operation-record" data-id="'+data+'">记录</a>';

                        if(row.item_status == 1)
                        {
                            $html_able = '<a class="btn btn-xs location--item-disable-submit" data-id="'+data+'">禁用</a>';
                        }
                        else
                        {
                            $html_able = '<a class="btn btn-xs location--item-enable-submit" data-id="'+data+'">启用</a>';
                        }

                        if(row.deleted_at == null)
                        {
                            $html_delete = '<a class="btn btn-xs location--item-delete-submit" data-id="'+data+'">删除</a>';
                        }
                        else
                        {
                            $html_delete = '<a class="btn btn-xs location--item-restore-submit" data-id="'+data+'">恢复</a>';
                        }

                        var html =
                            '<a class="btn btn-xs modal-show--for--location-item-edit" data-id="'+data+'">编辑</a>'+
                            $html_able+
                            $html_delete+
                            $html_operation_record+
                            // '<a class="btn btn-xs location--item-statistic" data-id="'+data+'">统计</a>'+
                            // '<a class="btn btn-xs location--item-login-submit" data-id="'+data+'">登录</a>'+
                            '';
                        return html;
            ...
            "drawCallback": function (settings) {
                ...
                console.log('location-list.datatable-query.execute');
