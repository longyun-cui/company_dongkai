<script>
    function Datatable__for__AI__Converted__Record__List($tableId)
    {
        let $that = $($tableId);
...
//                "scrollY": true,
            "scrollCollapse": true,
            "ajax": {
                'url': "{{ url('/o1/ai/ai-converted-record-list/datatable-query') }}",
                "type": 'POST',
                "dataType" : 'json',
                "data": function (d) {
...
//                    },
                ...
                },
                {
                    ...
                    render: function(data, type, row, meta) {
                        ...
                        {
                            $result_html = '<small class="btn-xs bg-blue">请求中</small>';
                        }
                        else if(data == 9)
                        {
                            $result_html = '<small class="btn-xs bg-green">已完成</small>';
                        }
                        else if(data == 19)
                        {
                            $result_html = '<small class="btn-xs bg-orange">未完成</small>';
                        }
                        else if(data == 99)
                        {
...
//                            return $year+'-'+$month+'-'+$day+'&nbsp;&nbsp;'+$hour+':'+$minute+':'+$second;
                ...
                },
                {
                    ...
                    render: function(data, type, row, meta) {
                        ...
                        if(row.item_status == 19)
                        {
                            $html_get = '<a class="btn btn-xs ai--converted--get-result" data-id="'+data+'">获取</a>';
                        }
