<script>
    $(function() {
        ...
        // 【搜索】
        $("#statistic-for-overview").on('click', ".filter-submit", function() {
            $('#datatable_ajax_channel').DataTable().ajax.reload();
            $('#datatable_ajax_project').DataTable().ajax.reload();
        });
        // 【搜索】【回车】
        $("#statistic-for-overview").on('keyup', ".filter-keyup", function(event) {
            if(event.keyCode ==13)
            {
                $("#statistic-for-overview").find(".filter-submit").click();
            }
        });
        // 【清空重选】
        $("#statistic-for-overview").on('click', ".filter-empty", function() {
            $("#statistic-for-overview").find('textarea.form-filter, input.form-filter, select.form-filter').each(function () {
                $(this).val("");
            });
...
//            $('select.form-filter').selectpicker('refresh');
            $("#statistic-for-overview").find('select.form-filter option').attr("selected",false);
            $("#statistic-for-overview").find('select.form-filter').find('option:eq(0)').attr('selected', true);
        });
        // 【刷新】
        $("#statistic-for-overview").on('click', ".filter-refresh", function() {
            $('#datatable_ajax').DataTable().ajax.reload(null,false);
        });
        // 【重置】
        $("#statistic-for-overview").on('click', ".filter-cancel", function() {
            $("#statistic-for-overview").find('textarea.form-filter, input.form-filter, select.form-filter').each(function () {
                $(this).val("");
            });
            $(".select2-box").val(-1).trigger("change");
...
//            $('select.form-filter').selectpicker('refresh');
            ...
            $("#statistic-for-overview").find('input[name=overview-time-type]').val('month');

            var $month_dom = $('input[name="overview-month"]');
            var $month_default = $month_dom.attr('data-default')
            $month_dom.val($month_default);

            var $date_dom = $('input[name="overview-date"]');
            var $date_default = $date_dom.attr('data-default')
            $date_dom.val($date_default);

            $(".statistic-title").html("【全部公司】");

            // $("#filter-submit-for-overview").click();
            $('#datatable_ajax_channel').DataTable().ajax.reload();
            $('#datatable_ajax_project').DataTable().ajax.reload();
        });
        ...
        // 全部搜索
        $(".main-content").on('click', "#filter-submit-for-overview", function() {

            $("#statistic-for-overview").find('input[name=service]').val('all');
        ...
        // 按【月】搜索
        $(".main-content").on('click', "#filter-submit-for-overview-by-month", function() {
            ...
            $(".statistic-time-type-title").html('按月查询');
            var $month_dom = $('input[name="overview-month"]');
        ...
        // 按【天】搜索
        $(".main-content").on('click', "#filter-submit-for-overview-by-date", function() {

            $("#statistic-for-overview").find('input[name=overview-time-type]').val('date');

            var $company_title = $('select[name=overview-company]').find("option:selected").text();
            var $statistic_title = "【" + $company_title + "】";
            $(".statistic-title").html($statistic_title);

            $(".statistic-time-type-title").html('按天查询');
            var $date_dom = $('input[name="overview-date"]');
        ...
        // 按【时间段】搜索
        $(".main-content").on('click', "#filter-submit-for-overview-by-period", function() {

            $("#statistic-for-overview").find('input[name=overview-time-type]').val('period');
            ...
            var $date_start = $('input[name="overview-start"]');
            var $date_ended = $('input[name="overview-ended"]');
        ...
        // 【前一月】
        $(".main-content").on('click', ".month-pick-pre-for-overview", function() {
            ...
            $("#filter-submit-for-overview-by-month").click();
            // $('#datatable_ajax').DataTable().ajax.reload();

        });
        // 【后一月】
        $(".main-content").on('click', ".month-pick-next-for-overview", function() {
        ...
        // 【前一天】
        $(".main-content").on('click', ".date-pick-pre-for-overview", function() {
            ...
            $("#filter-submit-for-overview-by-day").click();
            // $('#datatable_ajax').DataTable().ajax.reload();

        });
        // 【后一天】
        $(".main-content").on('click', ".date-pick-next-for-overview", function() {
    ...
    {
        var $company = $('select[name="overview-company"]').val();
        var $channel = $('select[name="overview-channel"]').val();
        var $project = $('select[name="overview-project"]').val();
        ...
        $.ajax({
            ...
            dataType:'json',
            async:false,
            url: "{{ url('/statistic/statistic-get-data-for-overview-of-daily-chart') }}",
            data: {
