<script>
    $(function() {
        ...
        // 【项目报表】全部搜索
        $(".main-content").on('click', "#filter-submit-for-company", function() {

            $("#company").find('input[name=company]').val('all');
            var $staff_type_title = $('select[name=company]').find("option:selected").text();
            $(".statistic-title").html($staff_type_title);
            $(".statistic-time-type-title").html('总量');
            $('#datatable_ajax').DataTable().ajax.reload();
        });
        // 【项目报表】按月搜索
        $(".main-content").on('click', "#filter-submit-for-company-by-month", function() {

            $("#statistic-for-company").find('input[name=company-time-type]').val('month');
            var $staff_type_title = $('select[name=company-staff-type]').find("option:selected").text();
            $(".statistic-title").html($staff_type_title);
            $(".statistic-time-type-title").html('按月');
            var $month_dom = $('input[name="company-month"]');
            var $the_month_str = $month_dom.val();
            $(".statistic-time-title").html('（'+$the_month_str+'月）');

            $('#datatable_ajax').DataTable().ajax.reload();

            statistic_get_data_for_company_chart($the_month_str,
        ...
        // 【项目报表】按天搜索
        $(".main-content").on('click', "#filter-submit-for-company-by-day", function() {

            $("#statistic-for-company").find('input[name=company-time-type]').val('day');
            var $staff_type_title = $('select[name=company-staff-type]').find("option:selected").text();
            $(".statistic-title").html($staff_type_title);
            $(".statistic-time-type-title").html('按天');
            var $date_dom = $('input[name="company-date"]');
            var $the_date_str = $date_dom.val();
            $(".statistic-time-title").html('（'+$the_date_str+'）');
            $('#datatable_ajax').DataTable().ajax.reload();
        });
        // 【项目报表】【重置】
        $("#statistic-for-company").on('click', ".filter-cancel", function() {
            $("#statistic-for-company").find('textarea.form-filter, input.form-filter, select.form-filter').each(function () {
                $(this).val("");
            });

//            $('select.form-filter').selectpicker('refresh');
            $("#statistic-for-company").find('select.form-filter option').attr("selected",false);
            $("#statistic-for-company").find('select.form-filter').find('option:eq(0)').attr('selected', true);

            $("#statistic-for-company").find('input[name=company-time-type]').val('');
            ...
            // $("#filter-submit-for-company").click();
            $('#datatable_ajax').DataTable().ajax.reload();
        ...
        // 【项目报表】【前一月】
        $(".main-content").on('click', ".month-pick-pre-for-company", function() {
            ...
            $("#filter-submit-for-company-by-month").click();
            // $('#datatable_ajax').DataTable().ajax.reload();

        });
        // 【项目报表】【后一月】
        $(".main-content").on('click', ".month-pick-next-for-company", function() {
        ...
        // 【项目报表】【前一天】
        $(".main-content").on('click', ".date-pick-pre-for-company", function() {
            ...
            $("#filter-submit-for-company-by-day").click();
            // $('#datatable_ajax').DataTable().ajax.reload();

        });
        // 【项目报表】【后一天】
        $(".main-content").on('click', ".date-pick-next-for-company", function() {
    ...
    function statistic_get_data_for_company_chart($month, $target_delivery_quantity, $target_cost_total ,$target_per_capita ,$target_unit_average)
    {
        var $company = $('select[name="company-channel"]').val();
        ...
        $.ajax({
            ...
            dataType:'json',
            async:false,
            url: "{{ url('/statistic/statistic-get-data-for-company-of-chart') }}",
            data: {
                _token: $('meta[name="_token"]').attr('content'),
                company: $company,
                month: $month,
                operate:"statistic-company"
            },
