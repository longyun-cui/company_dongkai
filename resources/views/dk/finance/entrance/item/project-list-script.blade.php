...
//                                $('#datatable_ajax').DataTable().ajax.reload(null,false);
        ...
        $(".main-content").on('click', ".item-modal-show-for-settle", function() {
            ...
            var $name = $row.find('.project-name').html();


            $('input[name="finance-create-project-id"]').val($id);
            $('.finance-create-project-name').html($name);
        ...
        $(".main-content").on('click', "#item-submit-for-finance-create", function() {
            ...
            layer.msg('确定"提交"么？', {
                ...
                ,yes: function(index){
                    ...
                    $.post(
                        "{{ url('/project/project-funds-using-create') }}",
                        {
                            _token: $('meta[name="_token"]').attr('content'),
