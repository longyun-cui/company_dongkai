...
//            window.location.href = "/statistic/statistic-user?id="+$that.attr('data-id');
        ...
        $("#modal-password-body").on('click', "#item-password-admin-change-submit", function() {
            ...
            layer.msg('确定"修改"么', {
                ...
                ,yes: function(index){
                    var options = {
                        url: "{{ url('/user/user-password-admin-change') }}",
                        type: "post",
                        dataType: "json",
                        // target: "#div2",
        ...
        $("#item-main-body").on('click', ".item-password-admin-reset-submit", function() {
            ...
            layer.msg('确定"重置"么', {
                ...
                ,yes: function(index){
                    $.post(
                        "{{ url('/user/user-password-admin-reset') }}",
                        {
                            _token: $('meta[name="_token"]').attr('content'),
                            operate: "user-password-admin-reset",
...
//                        window.open('/');
        ...
        $("#item-main-body").on('click', ".item-admin-restore-submit", function() {
            ...
            layer.msg('确定要"恢复"么？', {
                ...
                ,yes: function(index){
                    $.post(
                        "{{ url('/user/user-admin-restore') }}",
                        {
                            _token: $('meta[name="_token"]').attr('content'),
                            operate: "user-admin-restore",
                            user_id: $that.attr('data-id')
                        },
                        function(data){
                            layer.close(index);
        ...
        $("#item-main-body").on('click', ".item-admin-delete-permanently-submit", function() {
            ...
            layer.msg('确定"删除"么?', {
                ...
                ,yes: function(index){
                    $.post(
                        "{{ url('/user/user-admin-delete-permanently') }}",
                        {
                            _token: $('meta[name="_token"]').attr('content'),
                            operate: "user-admin-delete-permanently",
                            user_id: $that.attr('data-id')
                        },
                        function(data){
                            layer.close(index);
