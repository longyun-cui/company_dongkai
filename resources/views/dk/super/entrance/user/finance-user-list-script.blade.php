...
//                        window.open('/');
        ...
        // 【员工登录】
        $("#item-main-body").on('click', ".item-finance-login-submit", function() {
            var $that = $(this);
            $.post(
                "{{ url('/user/user-finance-login') }}",
                {
                    _token: $('meta[name="_token"]').attr('content'),
                    user_id: $that.attr('data-id'),
                ...
                function(data){
                    ...
                    else
                    {
                        console.log(data);
                        var temp_window=window.open();
                        temp_window.location = "{{ env('DOMAIN_DK_FINANCE') }}";
                    }
                },
                'json'
            );
        ...
        // 【客户登录】
        $("#item-main-body").on('click', ".item-client-login-submit", function() {
            var $that = $(this);
            $.post(
                "{{ url('/user/user-client-login') }}",
                {
                    _token: $('meta[name="_token"]').attr('content'),
                    user_id: $that.attr('data-id'),
