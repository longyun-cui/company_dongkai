...
//                        window.open('/');
        ...
        // 【客户登录】
        $("#item-main-body").on('click', ".item-customer-login-submit", function() {
            var $that = $(this);
            $.post(
                "{{ url('/user/user-customer-staff-login') }}",
                {
                    _token: $('meta[name="_token"]').attr('content'),
                    user_id: $that.attr('data-id'),
                    customer_id: $that.attr('data-id'),
                    type:'customer-staff'
                },
                function(data){
                    if(!data.success) layer.msg(data.msg);
                    else
                    {
                        console.log(data);
                        var temp_window=window.open();
                        temp_window.location = "{{ env('DOMAIN_DK_CUSTOMER') }}";
                    }
                },
                'json'
            );
