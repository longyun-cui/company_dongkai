<script>
    $(function() {
        ...
        $(".main-content").on('click', ".item-enable-by-admin-submit", function() {
            ...
            //
            $.post(
                "{{ url('/reconciliation/v1/operate/universal/item-enable-by-admin') }}",
                {
        ...
        $(".main-content").on('click', ".item-disable-by-admin-submit", function() {
            ...
            //
            $.post(
                "{{ url('/reconciliation/v1/operate/universal/item-disable-by-admin') }}",
                {
                    _token: $('meta[name="_token"]').attr('content'),
        ...
        $(".main-content").on('click', ".item-delete-by-admin-submit", function() {
            ...
            //
            $.post(
                "{{ url('/reconciliation/v1/operate/universal/item-delete-by-admin') }}",
                {
                    _token: $('meta[name="_token"]').attr('content'),
        ...
        $(".main-content").on('click', ".item-restore-by-admin-submit", function() {
            ...
            //
            $.post(
                "{{ url('/reconciliation/v1/operate/universal/item-restore-by-admin') }}",
                {
                    _token: $('meta[name="_token"]').attr('content'),
        ...
        $(".main-content").on('click', ".item-delete-permanently-by-admin-submit", function() {
            ...
            //
            $.post(
                "{{ url('/reconciliation/v1/operate/universal/item-delete-permanently-by-admin') }}",
                {
                    _token: $('meta[name="_token"]').attr('content'),
        ...
        // 【交易】删除
        $(".main-content").on('click', ".trade-submit-for-delete", function() {
            var $that = $(this);
            var $datatable_wrapper = $that.closest('.datatable-wrapper');
            var $item_category = $datatable_wrapper.data('datatable-item-category');
            var $table_id = $datatable_wrapper.find('table').filter('[id][id!=""]').attr("id");
            ...
            layer.msg('确定"弃用"么？', {
                ...
                ,yes: function(index){
                    ...
                    //
                    $.post(
                        "{{ url('/v1/operate/trade/item-delete') }}",
                        {
                            _token: $('meta[name="_token"]').attr('content'),
                            operate: "trade-item-delete",
                            item_category: $item_category,
                            item_id: $that.attr('data-id')
                        },
                        'json'
        ...
        // 【交易】确认
        $(".main-content").on('click', ".trade-submit-for-confirm", function() {
            var $that = $(this);
            var $datatable_wrapper = $that.closest('.datatable-wrapper');
            var $item_category = $datatable_wrapper.data('datatable-item-category');
            var $table_id = $datatable_wrapper.find('table').filter('[id][id!=""]').attr("id");
            ...
            layer.msg('确定"弃用"么？', {
                ...
                ,yes: function(index){
                    ...
                    //
                    $.post(
                        "{{ url('/v1/operate/trade/item-confirm') }}",
                        {
                            _token: $('meta[name="_token"]').attr('content'),
                            operate: "trade-item-confirm",
                            item_category: $item_category,
                            item_id: $that.attr('data-id')
                        },
                        'json'
        ...
        $(".main-content").on('click', ".item-modal-show-for-operation-record", function() {
            ...
            var $modal = $('#modal-for-operation-record-list');
            $modal.find('input[name="item-category"]').val($item_category);
