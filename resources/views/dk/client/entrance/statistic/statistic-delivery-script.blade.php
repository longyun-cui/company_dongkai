<script>
    $(function() {
        ...
        // 【搜索】
        $(".main-content").on('click', "#filter-submit-for-delivery", function() {

            $("#statistic-for-delivery").find('input[name=delivery]').val('all');
            var $staff_type_title = $('select[name=delivery]').find("option:selected").text();
            $(".statistic-title").html($staff_type_title);
            $(".statistic-time-type-title").html('总量');
            $('#datatable_ajax').DataTable().ajax.reload();
        });
        // 【搜索】【回车】
        $("#statistic-for-delivery").on('keyup', ".filter-keyup", function(event) {
            if(event.keyCode ==13)
            {
                $("#statistic-for-delivery").find(".filter-submit").click();
            }
        });
        // 【清空重选】
        $("#statistic-for-delivery").on('click', ".filter-empty", function() {
            $("#statistic-for-delivery").find('textarea.form-filter, input.form-filter, select.form-filter').each(function () {
                $(this).val("");
            });
...
//            $('select.form-filter').selectpicker('refresh');
            $("#statistic-for-delivery").find('select.form-filter option').attr("selected",false);
            $("#statistic-for-delivery").find('select.form-filter').find('option:eq(0)').attr('selected', true);
        });
        // 【刷新】
        $("#statistic-for-delivery").on('click', ".filter-refresh", function() {
            $('#datatable_ajax').DataTable().ajax.reload(null,false);
        });
        // 【重置】
        $("#statistic-for-delivery").on('click', ".filter-cancel", function() {
            $("#statistic-for-delivery").find('textarea.form-filter, input.form-filter, select.form-filter').each(function () {
                $(this).val("");
            });
...
//            $('select.form-filter').selectpicker('refresh');
            ...
            $("#statistic-for-delivery").find('input[name=delivery-time-type]').val('');

            var $month_dom = $('input[name="delivery-month"]');
            var $month_default = $month_dom.attr('data-default')
            $month_dom.val($month_default);

            var $date_dom = $('input[name="delivery-date"]');
            var $date_default = $date_dom.attr('data-default')
            $date_dom.val($date_default);

            $(".statistic-title").html("全部");

            // $("#filter-submit-for-delivery").click();
        ...
        // 【业务报表】按【月】搜索
        $(".main-content").on('click', "#filter-submit-for-delivery-by-month", function() {

            $("#statistic-for-delivery").find('input[name=delivery-time-type]').val('month');

            var $statistic_title = '';

            var $company = $('select[name=delivery-company]').find("option:selected");
            if($company.val() != "-1")
            {
            ...
            var $channel = $('select[name=delivery-channel]').find("option:selected");
            if($channel.val() != "-1")
            {
            ...
            var $business = $('select[name=delivery-business]').find("option:selected");
            if($business.val() != "-1")
            {
            ...
            var $project = $('select[name=delivery-project]').find("option:selected");
            if($project.val() != "-1")
            {
        ...
        $(".main-content").on('click', "#filter-submit-for-delivery-by-date", function() {
            ...
            $("#statistic-for-delivery").find('input[name=delivery-time-type]').val('date');
        ...
        $(".main-content").on('click', "#filter-submit-for-delivery-by-period", function() {
            ...
            $("#statistic-for-delivery").find('input[name=delivery-time-type]').val('period');
            ...
            var $date_start = $('input[name="delivery-start"]');
            var $date_ended = $('input[name="delivery-ended"]');
        ...
        // 【业务报表】【前一月】
        $(".main-content").on('click', ".month-pick-pre-for-delivery", function() {
        ...
        // 【业务报表】【后一月】
        $(".main-content").on('click', ".month-pick-next-for-delivery", function() {
        ...
        // 【业务报表】【前一天】
        $(".main-content").on('click', ".date-pick-pre-for-delivery", function() {
            ...
            $("#filter-submit-for-delivery-by-date").click();
            // $('#datatable_ajax').DataTable().ajax.reload();

        });
        // 【业务报表】【后一天】
        $(".main-content").on('click', ".date-pick-next-for-delivery", function() {
        ...
        $(".delivery-company").on("select2:select", function() {
            $(".delivery-channel").val(-1).trigger("change");
            $(".delivery-business").val(-1).trigger("change");
            $(".delivery-project").val(-1).trigger("change");
        });
        $(".delivery-channel").on("select2:select", function() {
            $(".delivery-company").val(-1).trigger("change");
            $(".delivery-business").val(-1).trigger("change");
            $(".delivery-project").val(-1).trigger("change");
        });
        $(".delivery-business").on("select2:select", function() {
            $(".delivery-company").val(-1).trigger("change");
            $(".delivery-channel").val(-1).trigger("change");
            $(".delivery-project").val(-1).trigger("change");
        });
        $(".delivery-project").on("select2:select", function() {
            $(".delivery-company").val(-1).trigger("change");
            $(".delivery-business").val(-1).trigger("change");
            $(".delivery-channel").val(-1).trigger("change");
        });
    ...
    function statistic_get_data_for_delivery_daily_chart($time_type, $time, $target_delivery_quantity, $target_cost_total ,$target_per_capita ,$target_unit_average)
    {
        var $company = $('select[name="delivery-company"]').val();
        var $channel = $('select[name="delivery-channel"]').val();
        var $project = $('select[name="delivery-project"]').val();
        ...
        $.ajax({
            ...
            dataType:'json',
            async:false,
            url: "{{ url('/statistic/statistic-get-data-for-delivery-of-daily-chart') }}",
            data: {
